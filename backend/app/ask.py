...
# Keywords that should never appear in a query we execute, even if it
# technically starts with SELECT (blocks stacked/multi-statement injection)
FORBIDDEN_KEYWORDS = [
...
def validate_query(query: str):
    """Raise ValueError if the query looks unsafe or malformed."""
    stripped = query.strip().rstrip(";").strip()

    if not stripped:
        raise ValueError("Empty query.")

    # Block stacked statements (a second statement after a semicolon)
    if ";" in stripped:
    ...
    # so this won't false-positive on a column named e.g. "updated_at")
...
def run_sql(conn, query: str):
    ...
    # Convert to plain JSON-safe values (dates, Decimals, etc. become strings)
    safe_rows = [
...
def build_chart(columns, rows):
    ...
    # Single value, single row — e.g. "SELECT COUNT(*) ..." → big stat number
    if len(columns) == 1 and len(rows) == 1:
     ...
     # Two-or-more-column result — the common "group by" shape.
    # We only chart the first two columns (label, value); extra columns
    if len(columns) >= 2 and len(rows) > 1:
        try:
            labels = [str(r[0]) for r in rows]
            values = [float(r[1]) for r in rows]

            # Date/time-like first column → line chart (trend over time)
            if _is_date_like(columns[0]):
                return {"kind": "line", "title": f"{columns[1]} over {columns[0]}", "labels": labels, "values": values}

            # Few categories → pie chart reads better than a bar
            if len(labels) <= 6:
                return {"kind": "pie", "title": f"{columns[1]} by {columns[0]}", "labels": labels, "values": values}

            # Many categories → bar chart stays more readable than a pie
            return {"kind": "bar", "title": f"{columns[1]} by {columns[0]}", "labels": labels, "values": values}
        except (TypeError, ValueError):
            pass

    # Fallback — anything else (multi-column detail rows, mixed types) → table
    return {
        "kind": "table",
        "title": "Query results",
        "labels": [],
...
def pick_best_query(query_history):
    """Prefer the richest query result (grouped/multi-row) over simple sanity checks."""
...
def ask(payload: AskRequest):
    ...
    # Open read-only at the SQLite level as a second line of defense —
    # even if a bad query somehow got through validation, the connection
    # itself physically cannot write.
        ...
        for _ in range(MAX_LOOP_ITERATIONS):
            try:
                ...
                if e.status_code == 429:
                    raise HTTPException(
                        status_code=429,
                        detail="You've hit the free-tier rate limit. Wait a moment and try again.",
                    )
