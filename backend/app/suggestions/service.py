...
# output via response_schema, same shared LLMRateLimitError/
# LLMOverloadedError/LLMServiceError types so callers (suggestions/api.py)
# can handle this failure the same way every other LLM call in the app is
# handled.
#
...
def polish_questions(
    ...
    in the same order. Raises LLMRateLimitError / LLMOverloadedError /
    LLMServiceError on a Gemini API failure -- same types as
    planner/service.py and explainer/service.py, so this can share their
    except blocks.
